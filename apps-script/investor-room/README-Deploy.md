...
A data room in the style of Papermark: an email-gated link per investor, a confidentiality click-through, page-by-page time tracking and an instant kill switch. It runs on Google Sheets and Apps Script under **admin@studiocahaya.com**, costs ₹0 and needs no new vendor.
...
## One-time setup (≈10 min, signed in as admin@studiocahaya.com)
...
3. Reload the Sheet. A **Yello Room** menu appears. Run **Yello Room → Set up room (first run)** and approve the permissions it asks for (Sheets, Drive, mail, triggers). This step:
   ...
   - sends alerts to the account that runs it. To send them somewhere else, change `ALERT_EMAIL` under **Project Settings → Script properties**.
